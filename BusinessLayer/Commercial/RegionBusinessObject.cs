...
namespace Recodme.RD.FullStoQReborn.BusinessLayer.Commercial
{
    public class RegionBusinessObject
    {
        ...
        public OperationResult<bool> Create(Region item)
        {
            try
            {
                if (_dao.List().Any(x => x.Name.ToLower() == item.Name.ToLower() && !x.IsDeleted)) return new OperationResult<bool>() { Success = true, Result = false, Message = "Name already exists" };
                _dao.Create(item);
        ...
        public async Task<OperationResult<bool>> CreateAsync(Region item)
        {
            try
            {
                if (_dao.ListAsync().Result.Any(x => x.Name.ToLower() == item.Name.ToLower() && !x.IsDeleted)) return new OperationResult<bool>() { Success = true, Result = false, Message = "Name already exists" };
        ...
        public OperationResult<bool> Update(Region item)
        {
            try
            {
                if (_dao.List().Any(x => x.Name.ToLower() == item.Name.ToLower() && x.Id != item.Id && !x.IsDeleted)) return new OperationResult<bool>() { Success = true, Result = false, Message = "Name already exists" };
                _dao.Update(item);
                return new OperationResult<bool>() { Success = true, Result = true };
        ...
        public async Task<OperationResult<bool>> UpdateAsync(Region item)
        {
            try
            {
                if (_dao.ListAsync().Result.Any(x => x.Name.ToLower() == item.Name.ToLower() && x.Id != item.Id && !x.IsDeleted)) return new OperationResult<bool>() { Success = true, Result = false, Message = "Name already exists" };
        ...
        #region Filter
        public OperationResult<List<Region>> Filter(Func<Region, bool> predicate)
        {
            try
            {
                ...
                transactionScope.Complete();
                return new OperationResult<List<Region>> { Result = result, Success = true };
            }
            catch (Exception e)
            {
                return new OperationResult<List<Region>>() { Success = false, Exception = e };
        ...
        public async Task<OperationResult<List<Region>>> FilterAsync(Func<Region, bool> predicate)
        {
            try
            {
                TransactionOptions transactionOptions = new TransactionOptions
